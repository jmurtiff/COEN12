...
/* individual words within a SET along with a sorted array of all */
 ...
 *
 * Efficiency: O(n) with O(1) being the ideal case
 * 
...
/*
 * Function: The main function used in the quickSort function. The function takes in
 * a pointer to an array of void pointers and then partitions an array of void pointers
 * into three parts (elements less than the pivot, the pivot itself, and elements greater 
 * than the pivot). Then, the function loops through the entire given sub-aray (since 
 * the parameters of either high or low are reduced as the array becomes more sorted)
 * and then swaps elements that are less than the pivot's value with the position of 
 * low's index (which increments with each swap. Finally, once the entire sub-array 
 * has been partioned, the pivot's value is swapped with the last value that we swapped with
 * in the array.
 *
 * Efficiency: O(n)
 *
 * Return Value: The index of the pivot's new location in the array of void pointers 
 * (once partitioning has completed on the two sub-arrays).
 */
int partition(void** vp, SET *sp, int low, int high)
{
	assert(vp != NULL);
	void* pivot = vp[high];
	int i = low - 1;
...
/*
 * Function: A quicksorting algorithim that recursively sorts an array of void pointers 
 * in ascending order (from least to greatest). The function continually calls itself 
 * until the entire array of data has been sorted (when the first index "low" is greater 
 * than the last index "high"). First the function calls the partition function which takes
 * the last element of the array of void pointers as the pivot and then places all elements 
 * less than the pivot value to the left of it in the array and all elements that are greater
 * than the pivot to the right of it in the array. Once this has been completed, the pivot 
 * element has now been correctly sorted. Next, the function does a recursive quicksort on 
 * the left and right sub-arrays of the original pivot element. Once these sub-arrays have 
 * been sorted (the quickSort function has been caled until there is only one element left in 
 * each sub-array) then the array of void pointers has been sorted.
 *
 * Efficiency: O(n^2)
 *
 * Return Value: No return value.
 */
void quickSort(void* vp, SET *sp, int low, int high)
{
	assert(vp != NULL);
	if(low < high)
	{
		int part = partition(vp,sp,low,high);
		quickSort(vp,sp,low,part-1);
		quickSort(vp,sp,part+1,high);
	}
}
 ...
 * over to an allocated array of void pointers. This newly allocated array contains 
 * no empty spaces after going through the entirety of our SET and only contains 
 * only the character array pointers of the filled elements of our SET.
 * Additionally, our array of void pointers is sorted using Quicksort before 
...
void *getElements(SET *sp)
{
	...
	quickSort(dataSet,sp,0,sp->count-1);
	return dataSet; 	
}
